...
async def upload_audio(file: UploadFile = File(...)):
    """
    Accepts an audio blob (audio/webm, audio/wav). For hackathon demo we POST to AIPipe's REST
    transcription endpoint (Whisper-like). Returns JSON with 'transcript'.
    """
    if not AIPIPE_API_KEY:
